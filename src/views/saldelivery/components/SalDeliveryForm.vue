    ...
    <a-tabs v-model:activeKey="activeKey" animated  @change="handleChangeTabs">
          <a-tab-pane tab="销售发货_明细" key="salDeliveryDetail" :forceRender="true">
            <JVxeTable
              ...
              v-if="salDeliveryDetailTable.show"
...
<script lang="ts">
  ...
  import {getBpmFormSchema,salDeliveryDetailColumns} from '../SalDelivery.data';
  import {saveOrUpdate,salDeliveryDetailList} from '../SalDelivery.api';

  export default defineComponent({
    name: "SalDeliveryForm",
    components:{
      BasicForm,
    },
    props:{
    ...
    setup(props){
      ...
      const queryByIdUrl = '/saldelivery/salDelivery/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(salDeliveryDetailList, {id: data.id}, salDeliveryDetailTable, ()=>{
          salDeliveryDetailTable.show = true;
        });
        //默认是禁用
