    ...
    <a-tabs v-model:activeKey="activeKey" animated  @change="handleChangeTabs">
          <a-tab-pane tab="销售结算_明细" key="salSettleDetail" :forceRender="true">
            <JVxeTable
              ...
              resizable
              ref="salSettleDetail"
              v-if="salSettleDetailTable.show"
...
<script lang="ts">
  ...
  import {getBpmFormSchema,salSettleDetailColumns} from '../SalSettle.data';
  ...
  export default defineComponent({
    ...
    setup(props){
      ...
      const queryByIdUrl = '/salsettle/salSettle/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(salSettleDetailList, {id: data.id}, salSettleDetailTable, ()=>{
          salSettleDetailTable.show = true;
        });
        //默认是禁用
