...
<script lang="ts">
  ...
  import {getBpmFormSchema,salQuoteDetailColumns} from '../SalQuote.data';
  ...
  export default defineComponent({
    ...
    setup(props){
      ...
      const queryByIdUrl = '/salquote/salQuote/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(salQuoteDetailList, {id: data.id}, salQuoteDetailTable, ()=>{
          salQuoteDetailTable.show = true;
        });
        //默认是禁用
