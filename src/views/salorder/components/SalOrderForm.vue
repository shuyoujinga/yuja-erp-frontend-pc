...
<script lang="ts">
  ...
  import {getBpmFormSchema,salOrderDetailColumns} from '../SalOrder.data';
  import {saveOrUpdate,salOrderDetailList} from '../SalOrder.api';
  ...
  export default defineComponent({
    ...
    setup(props){
      ...
      const queryByIdUrl = '/salorder/salOrder/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(salOrderDetailList, {id: data.id}, salOrderDetailTable, ()=>{
          salOrderDetailTable.show = true;
        });
        //默认是禁用
