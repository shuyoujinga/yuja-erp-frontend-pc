    ...
    <a-tabs v-model:activeKey="activeKey" animated  @change="handleChangeTabs">
          <a-tab-pane tab="生产报工_明细" key="prdReportDetail" :forceRender="true">
            <JVxeTable
              ...
              resizable
              ref="prdReportDetail"
              v-if="prdReportDetailTable.show"
...
<script lang="ts">
  ...
  import {getBpmFormSchema,prdReportDetailColumns} from '../PrdReport.data';
  ...
  export default defineComponent({
    ...
    setup(props){
      ...
      const queryByIdUrl = '/prdreport/prdReport/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(prdReportDetailList, {id: data.id}, prdReportDetailTable, ()=>{
          prdReportDetailTable.show = true;
        });
        //默认是禁用
