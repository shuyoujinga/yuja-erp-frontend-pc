    ...
    <a-tabs v-model:activeKey="activeKey" animated  @change="handleChangeTabs">
          <a-tab-pane tab="库存盘点_明细" key="invStockTakeDetail" :forceRender="true">
            <JVxeTable
              ...
              v-if="invStockTakeDetailTable.show"
...
<script lang="ts">
  ...
  import {getBpmFormSchema,invStockTakeDetailColumns} from '../InvStockTake.data';
  import {saveOrUpdate,invStockTakeDetailList} from '../InvStockTake.api';

  export default defineComponent({
    name: "InvStockTakeForm",
    components:{
      BasicForm,
    },
    props:{
    ...
    setup(props){
      ...
      const queryByIdUrl = '/invstocktake/invStockTake/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(invStockTakeDetailList, {id: data.id}, invStockTakeDetailTable, ()=>{
          invStockTakeDetailTable.show = true;
        });
        //默认是禁用
