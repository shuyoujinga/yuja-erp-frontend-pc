...
<script lang="ts" setup>
    ...
    import {formSchema,purSupplierQuotationDetailColumns} from '../PurSupplierQuotation.data';
    ...
    const [registerModal, {setModalProps, closeModal}] = useModalInner(async (data) => {
        ...
        if (unref(isUpdate)) {
            ...
            await setFieldsValue({
             ...
             requestSubTableData(purSupplierQuotationDetailList, {id:data?.record?.id}, purSupplierQuotationDetailTable)
        }
        // 隐藏底部时禁用整个表单
    ...
    async function reset(){
      await resetFields();
      activeKey.value = 'purSupplierQuotationDetail';
      purSupplierQuotationDetailTable.dataSource = [];
    }
    ...
    function handleValueChange({ row, column, value }) {
      ...
      if ((column.key === 'quotationQty'||column.key === 'unitPrice') && value) {
        row.amount = (Number(row.quotationQty) || 0) * (Number(row.unitPrice) || 0);
      }
    }
