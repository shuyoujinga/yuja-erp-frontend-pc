...
<script lang="ts">
  ...
  import {getBpmFormSchema,purApplyDetailColumns} from '../PurApply.data';
  ...
  export default defineComponent({
    ...
    setup(props){
      ...
      const queryByIdUrl = '/purapply/purApply/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(purApplyDetailList, {id: data.id}, purApplyDetailTable, ()=>{
          purApplyDetailTable.show = true;
        });
        //默认是禁用
