...
export function useMethods() {
   ...
   * 审批/反审核请求
   * @param url 审批接口地址
   * @param data 请求体，符合后端 AuditRequest 格式
   */
  async function audit(url: string, data: { ids: (string | number)[], type: string }) {
    if (!url) {
      console.error('审批接口 URL 未配置');
      return;
    }

    if (!data?.ids?.length) {
      createMessage.warning('请选择要操作的记录');
      return;
    }
     await defHttp.post({ url, data });
  ...
  return {
    ...
    handleAudit: (url: string, data: { ids: (string | number)[], type: string }) => audit(url, data),
  };
}
