<template>
  <BasicModal v-bind="$attrs" @register="registerModal" destroyOnClose :title="title" :width="1800" @ok="handleSubmit">
...
<script lang="ts" setup>
    ...
    import {formSchema,prdWorkOrderDetailColumns} from '../PrdWorkOrder.data';
    ...
    const [registerModal, {setModalProps, closeModal}] = useModalInner(async (data) => {
        ...
        if (unref(isUpdate)) {
            ...
            await setFieldsValue({
             ...
             requestSubTableData(prdWorkOrderDetailList, {id:data?.record?.id}, prdWorkOrderDetailTable)
        }
        // 隐藏底部时禁用整个表单
    ...
    async function reset(){
      await resetFields();
      activeKey.value = 'prdWorkOrderDetail';
      prdWorkOrderDetailTable.dataSource = [];
    }
