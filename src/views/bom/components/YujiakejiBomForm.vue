    ...
    <a-tabs v-model:activeKey="activeKey" animated  @change="handleChangeTabs">
          <a-tab-pane tab="材料清单_明细表" key="yujiakejiBomDetail" :forceRender="true">
            <JVxeTable
              ...
              v-if="yujiakejiBomDetailTable.show"
...
<script lang="ts">
  ...
  import {getBpmFormSchema,yujiakejiBomDetailColumns} from '../YujiakejiBom.data';
  import {saveOrUpdate,yujiakejiBomDetailList} from '../YujiakejiBom.api';

  export default defineComponent({
    name: "YujiakejiBomForm",
    components:{
      BasicForm,
    },
    props:{
    ...
    setup(props){
      ...
      const queryByIdUrl = '/bom/yujiakejiBom/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(yujiakejiBomDetailList, {id: data.id}, yujiakejiBomDetailTable, ()=>{
          yujiakejiBomDetailTable.show = true;
        });
        //默认是禁用
