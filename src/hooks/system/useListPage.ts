...
const { handleExportXls, handleImportXls,handleAudit } = useMethods();
...
export function useListPage(options: {
  importConfig: { success: () => void; url: Api };
  exportConfig: { name: string; params: any; url: Api };
  auditConfig: { success: () => void; url: Api };
  tableProps: {
    actionColumn: { width: number; fixed: string };
    columns: BasicColumn[];
    formConfig: {
   ...
   * @param selectedIds
   */
  async function onAudit(auditType: string, selectedIds: (string | number)[]) {
    if (!selectedIds || !selectedIds.length) {
      return $message.createMessage.warn('请选择要操作的记录');
    }

    const url = options.auditConfig?.url;
    if (!url) {
      return console.error(`未配置 ${auditType} URL`);
    }

    // 构造请求体，符合后端 AuditRequest
    const requestBody = {
      ids: selectedIds,
      type: auditType
    };

    await handleAudit(url, requestBody);
