<template>
  ...
  <!-- 子表单区域 -->
    <a-tabs v-model:activeKey="activeKey" animated @change="handleChangeTabs">
      <a-tab-pane tab="采购收货_明细" key="purReceiveDetail" :forceRender="true">
        <JVxeTable
          keep-source
          resizable
          ref="purReceiveDetail"
...
<script lang="ts" setup>
    ...
    import {formSchema,purReceiveDetailColumns} from '../PurReceive.data';
    import {saveOrUpdate, purReceiveDetailList, purReceiveDetailListByIds} from '../PurReceive.api';
    ...
    const refKeys = ref(['purReceiveDetail', ]);
    const activeKey = ref('purReceiveDetail');
    const purReceiveDetail = ref();
    const tableRefs = {purReceiveDetail, };
    const purReceiveDetailTable = reactive({
          loading: false,
          dataSource: [],
          columns:purReceiveDetailColumns
    })
    //表单配置
    ...
    const [registerModal, {setModalProps, closeModal}] = useModalInner(async (data) => {
        ...
        if (unref(isUpdate)) {
            ...
            await setFieldsValue({
             ...
             requestSubTableData(purReceiveDetailList, {id:data?.record?.id}, purReceiveDetailTable)
        }
        // 隐藏底部时禁用整个表单
    ...
    async function reset(){
      await resetFields();
      activeKey.value = 'purReceiveDetail';
      purReceiveDetailTable.dataSource = [];
    }
    function classifyIntoFormData(allValues) {
         let main = Object.assign({}, allValues.formValue)
         return {
           ...main, // 展开
           purReceiveDetailList: allValues.tablesValue[0].tableData,
         }
       }
    //表单提交事件
    ...
    function handleFormChange(changedValues, allValues) {
      ...
      if (changedValues.orderDetailId !== undefined) {
        ...
        // 用户真正修改 → 必定触发
        requestSubTableData(
          purReceiveDetailListByIds,
          { id: changedValues.orderDetailId },
          purReceiveDetailTable
        );
      }
    }
    ...
    function handleValueChange({ row, column, value }) {
      ...
      if ((column.key === 'receiveQty'||column.key === 'unitPrice') && value) {
        row.amount = (Number(row.receiveQty) || 0) * (Number(row.unitPrice) || 0);
      }
    }
</script>
