    ...
    <a-tabs v-model:activeKey="activeKey" animated  @change="handleChangeTabs">
          <a-tab-pane tab="销售收款_明细" key="salReceiptDetail" :forceRender="true">
            <JVxeTable
              ...
              v-if="salReceiptDetailTable.show"
...
<script lang="ts">
  ...
  import {getBpmFormSchema,salReceiptDetailColumns} from '../SalReceipt.data';
  ...
  export default defineComponent({
    ...
    setup(props){
      ...
      const queryByIdUrl = '/salreceipt/salReceipt/queryById';
      async function initFormData(){
        let params = {id: props.formData.dataId};
        const data = await defHttp.get({url: queryByIdUrl, params});
        //设置表单的值
        await setFieldsValue({...data});
        requestSubTableData(salReceiptDetailList, {id: data.id}, salReceiptDetailTable, ()=>{
          salReceiptDetailTable.show = true;
        });
        //默认是禁用
